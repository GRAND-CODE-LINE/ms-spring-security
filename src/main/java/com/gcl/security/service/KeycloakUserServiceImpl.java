...
import com.gcl.dental.core.model.dental.Symbol;
...
public class KeycloakUserServiceImpl implements KeycloakUserService  {
	 ...
	 */
	public UserListDTO findAllUsers(Map<String, String> filters) {
		ObjectMapper mapper = new ObjectMapper();
		Symbol pojo = mapper.convertValue(filters, Symbol.class);
		...
		UserListDTO userListDTO = new UserListDTO(KeycloakProvider.getRealmResource().users().list(page, size), 
				KeycloakProvider.getRealmResource().users().count());
	...
	public String createUser(@NonNull UserDTO userDTO) {
		...
		if (status == 201) {
			...
			if (userDTO.getRoles() == null || userDTO.getRoles().isEmpty()) {
				...
				rolesRepresentation = realmResource.roles().list().stream().filter(role -> userDTO.getRoles().stream()
